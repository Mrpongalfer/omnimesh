...
Primary autonomous execution engine for the UMCC system
...
class AgentExWork:
    """
    The Genesis Agent - Primary autonomous execution engine for UMCC
    """

    def __init__(self, config_path: Optional[str] = None):
        self.config_path = config_path or "config/agent_config.toml"
        ...
        logger.info("🧠 Agent Ex-Work v3.0 initializing...")
    ...
    def load_config(self) -> None:
        """Load agent configuration from TOML files"""
        logger.info("📋 Loading Agent Ex-Work configuration...")

        # Try to load agent-specific config first
        agent_config_path = Path(self.config_path)
        global_config_path = Path("../config/config.toml")

        config_data = {}

        # Load global UMCC config
        if global_config_path.exists():
            try:
                global_config = toml.load(global_config_path)
                logger.info(f"✅ Loaded global config from {global_config_path}")

                # Extract agent-relevant settings
                if "hive" in global_config:
                    config_data["hive_address"] = global_config["hive"].get("primary_hive_address", "localhost:50051")

                if "ollama" in global_config:
                    config_data["ollama_host"] = global_config["ollama"].get("ollama_host", "http://localhost:11434")

            except Exception as e:
                logger.warning(f"⚠️  Failed to load global config: {e}")
